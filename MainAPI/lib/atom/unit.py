...
class Lation_unit :
    ...
    def __init__(self,data) -> None:
        ...
        _ref = self.db.execute_query('SELECT * FROM tb_divion_ref WHERE type=%s',(self._info["type"],))[0]
        for k in _ref.keys() :
            self._spec[k] = _ref[k]
        return
    ...
    def save_unit_stat_db(self) -> None :
        ...
        if self._stat["hp"] == 0 :
            self.db.execute_query('DELETE FROM tb_user_divion WHERE unitno = %s',(self._info["unitno"],))
        else :
